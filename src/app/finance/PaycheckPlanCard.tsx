...
export function PaycheckPlanCard({
  ...
  const debtByAccountId = new Map(debtSummaries.map((d) => [d.account.id, d]));
      ...
      <div className="border-t border-neutral-800 pt-4">
        ...
        <div className="space-y-2">
          {initialBills.map((bill) => {
            const debt = bill.accountId ? debtByAccountId.get(bill.accountId) : undefined;
            return (
              <div key={bill.id} className="rounded-md border border-neutral-800 p-3">
                ...
                {debt && (
                  <div className="mt-2 flex items-center justify-between text-xs text-neutral-500">
                    <span>
                      {formatCurrency(debt.balance)} balance · {(debt.apr * 100).toFixed(2)}% APR
                    </span>
                    <span>Payoff: {formatDate(debt.projectedDate)}</span>
                  </div>
                )}
              </div>
            );
